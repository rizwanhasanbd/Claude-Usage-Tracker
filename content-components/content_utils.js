...
async function Log(...args) {
	...
	// If the runtime is gone, silently bail — the page will recover on next reload.
	let result;
	try {
		if (!browser?.runtime?.id) return;          // context already dead
		result = await browser.storage.local.get('debug_mode_until');
	} catch (e) {
		return;                                      // storage call threw, bail quietly
	}
